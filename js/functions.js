...
async function processFunction(command, stateMachine, bot) {
    ...
    switch (command.name) {
        ...
        case "trench":
            ...
            break
        case "tunnel":
            // TODO this could be the same state as trench
            stateMachine.clear()
            stateMachine.transition(new Tunnel(command.parameters))
            response = "Tunneling"
            break
        case "dig":
        ...
        case "get_status":
            async function reportStatus() {
                ...
                return `Operating at ${status.efficiency} efficiency, power level is ${status.powerLevel}, location is ${status.location.x} ${status.location.y} ${status.location.z} in ${status.location.dimension} and current task is ${status.task}`
            }
